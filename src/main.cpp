...
#if defined(NDEBUG)
# error "Taskcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x704c13a1d724eb47be6b0be2b5ea1ce1529f5b6c8b7bc289c74e72c1f88ef4a9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Taskcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Taskcoin Signed Message:\n";
...
{
    // Taskcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 800000); // Taskcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Taskcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Taskcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Taskcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Taskcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0ce5b589b053118334472c22c9a32e7b3ff0b94bec770ea20fb439ecf06d093f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b212e9399103befa750e656af473ca09cb216b528612b494469054db13cf98112a9c0649d13dcb4df03a1372549d2c46c572804574df98b96fae020756a73878") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516446856;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085341271;

        if (fTestNet)
        {
            block.nTime    = 1516446823;
            block.nNonce   = 385806990;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xaee709c81d1862227f286a9df300cc8351c775d01998c5ade8da0aac4ad1304b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc3, 0xba, 0xd3 }; // Taskcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TaskcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TaskcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TaskcoinMiner(CWallet *pwallet)
{
    printf("TaskcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("taskcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TaskcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TaskcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TaskcoinMiner, pwallet));
}
